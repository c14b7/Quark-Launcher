...
import { Client, Databases, Permission, Role, IndexType, Storage } from 'node-appwrite';
  ...
  switch (attr.type) {
    ...
    case 'email':
      await databases.createEmailAttribute(
        ...
        attr.default as string | undefined, attr.array || false
      );
      break;
    case 'integer':
      await databases.createIntegerAttribute(
        DATABASE_ID, collectionId, attr.key, attr.required,
        undefined, undefined, attr.default as number | undefined, attr.array || false
      );
      break;
    case 'boolean':
  ...
  for (const collection of COLLECTIONS) {
    ...
    try {
      await databases.createCollection(
        DATABASE_ID, collection.id, collection.name, SERVER_ONLY_PERMISSIONS
      );
      console.log('   ✅ Created');
    } catch (error: unknown) {
      const e = error as { code?: number };
      if (e.code === 409) console.log('   ℹ️  Already exists');
      else { console.error('   ❌', error); continue; }
    }
    ...
    for (const attr of collection.attributes) {
      try {
        ...
        const e = error as { code?: number };
        if (e.code === 409) console.log(`   📝 ${attr.key} ℹ️`);
        else console.error(`   📝 ${attr.key} ❌`, error);
      }
    }
    console.log('');
  }
