...
async function createAttr(databases, collectionId, attr) {
    switch (attr.type) {
        case 'string':
            await databases.createStringAttribute(DATABASE_ID, collectionId, attr.key, attr.size, attr.required, undefined, attr.array || false);
            break;
        case 'datetime':
            await databases.createDatetimeAttribute(DATABASE_ID, collectionId, attr.key, attr.required);
            break;
        case 'enum':
            await databases.createEnumAttribute(DATABASE_ID, collectionId, attr.key, attr.elements, attr.required, attr.required ? undefined : attr.default, false);
            break;
        case 'boolean':
            await databases.createBooleanAttribute(DATABASE_ID, collectionId, attr.key, attr.required, attr.required ? undefined : attr.default, false);
            break;
    }
}
async function main() {
    ...
    for (const col of COLLECTIONS) {
        try {
            ...
            const err = e;
            if (err.code === 409)
                console.log(`ℹ️  collection ${col.id}`);
            else
                throw e;
        }
        for (const attr of col.attributes) {
            try {
                ...
                const err = e;
                if (err.code === 409)
                    console.log(`   ℹ️  ${col.id}.${attr.key}`);
                else
                    console.error(`   ❌ ${col.id}.${attr.key}`, e);
            }
        }
    }
    const indexes = [
    ...
    for (const idx of indexes) {
        try {
            await databases.createIndex(DATABASE_ID, idx.collection, idx.key, idx.unique ? node_appwrite_1.IndexType.Unique : node_appwrite_1.IndexType.Key, idx.attributes);
            console.log(`✅ index ${idx.key}`);
        }
        catch (e) {
            const err = e;
            if (err.code === 409)
                console.log(`ℹ️  index ${idx.key}`);
            else
                console.error(`❌ index ${idx.key}`, e);
        }
    }
    console.log('\n🎉 Chat migration done');
